  ...
  return (
    <section className="screen" aria-labelledby="market-title">
      ...
      <div className="market-grid">
        <section className="market-hero"><p className="overline">TENDÊNCIA ATUAL</p><span className="market-arrow">↗</span><h3>{labelOf(state.world.knownGenres, state.market.genre)}</h3><strong>{angle.label}</strong><p>{angle.copy}</p><div className="heat-meter"><span style={{ width: `${state.market.heat}%` }} /></div><small>Mais {state.market.monthsLeft} meses, se a previsão estiver certa.</small></section>
