    ...
    <Modal open={projectModalOpen} onClose={() => setProjectModalOpen(false)} className="project-modal project-modal-expanded" label="Novo projeto">
      <form onSubmit={submit}>
        ...
        <div className="project-form">
          ...
          <label className="select-field"><span>11 · EQUIPE DE PRODUÇÃO</span><select value={form.productionUnitId} onChange={event => update('productionUnitId', event.target.value)}>{units.map(unit => <option key={unit.id} value={unit.id}>{unit.name} · força {unit.skill}</option>)}</select><small>{state.currentProject ? 'Times secundários progridem sozinhos a cada mês.' : 'A primeira frente sempre usa a equipe principal.'}</small></label>

          <label className="select-field"><span>12 · ACESSÓRIO</span><select value={form.accessoryId} onChange={event => update('accessoryId', event.target.value)}><option value="">Nenhum</option>{accessories.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><small>{form.accessoryId ? accessories.find(item => item.id === form.accessoryId)?.copy : 'Somente periféricos relevantes da plataforma e da época aparecem aqui.'}</small></label>

          {motionAvailable && <label className="select-field"><span>13 · CONTROLES DE MOVIMENTO</span><select value={form.controlScheme} onChange={event => update('controlScheme', event.target.value)}>{CONTROL_SCHEMES.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select><small>No Wii, por exemplo, movimento pode ser obrigatório, opcional ou ignorado.</small></label>}

          {!type.requiresFranchise && <label className="select-field"><span>FRANQUIA PRÓPRIA</span><select value={form.franchiseId} onChange={event => chooseFranchise(event.target.value)}><option value="">Nova franquia</option>{franchises.map(item => <option key={item.id} value={item.id}>Usar {item.name}</option>)}</select><small>Útil para remake, remaster, port e coletânea ligados a uma série existente.</small></label>}
