    ...
    <WorkbenchCard kicker="PLANO DE LANÇAMENTO" meta={project.announced ? `HYPE ${project.hype}` : 'NÃO ANUNCIADO'} className="strategy-card">
      <div className="strategy-body">
        ...
        <section className="publisher-slot">
          {project.publisher ? <><span>EDITORA</span><h3>{project.publisher.name}</h3><p>Mais alcance. {Math.round(project.publisher.royalty * 100)}% da sua parte fica com a editora.</p></> : offer ? <><span>PROPOSTA NA MESA</span><h3>{offer.name}</h3><p>{offer.demand}</p><div><strong>Adiantamento {formatMoney(offer.advance)}</strong><small>{Math.round(offer.royalty * 100)}% de participação · alcance ×{offer.reach}</small></div><div className="publisher-actions"><Button variant="primary" size="small" onClick={() => dispatch({ type: 'ACCEPT_PUBLISHER', offerId: offer.id })}>ASSINAR CONTRATO</Button><Button size="small" onClick={() => dispatch({ type: 'REJECT_PUBLISHER', offerId: offer.id })}>RECUSAR {formatMoney(offer.advance)}</Button></div></> : project.announced ? <><span>EDITORA</span><h3>{project.publisherDeclines ? `${project.publisherDeclines} proposta${project.publisherDeclines > 1 ? 's' : ''} recusada${project.publisherDeclines > 1 ? 's' : ''}` : 'Sem proposta'}</h3><p>Você pode procurar uma. Dinheiro alto continua sendo opcional.</p><Button size="small" onClick={() => dispatch({ type: 'REQUEST_PUBLISHER' })}>PROCURAR EDITORA</Button></> : null}
        </section>
      </div>
    </WorkbenchCard>
  )
