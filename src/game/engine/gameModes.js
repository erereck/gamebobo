...
export const GAME_MODES = Object.freeze([
  {
    id: 'traditional',
    label: 'Tradicional',
    kicker: 'A CARREIRA ORIGINAL',
    difficulty: 'PADRÃO',
    description: 'O Gamebobo exatamente como sempre foi. Sem restrições extras e sem atalhos.',
    rules: ['Todas as plataformas', 'Todas as escalas', 'Economia e prêmios padrão'],
  },
  {
    id: 'realistic',
    label: 'Realista',
    kicker: 'PRESTÍGIO CUSTA CARO',
    difficulty: 'DIFÍCIL',
    description: 'As vendas continuam iguais, mas sobra menos dinheiro. Notas acima de 90 e troféus viram acontecimentos raros.',
    rules: ['Receita de jogos -30%', '90+ bem mais raro', 'Premiações mais exigentes'],
  },
  {
    id: 'portable',
    label: 'Portátil',
    kicker: 'SÓ CABE NO BOLSO',
    difficulty: 'ESPECIAL',
    description: 'Seu estúdio só trabalha com hardware portátil. Consoles de mesa, PC, mobile e arcade ficam fora do catálogo.',
    rules: ['Só handhelds e híbridos', 'Começa a partir de 1989', 'Multiplataforma só entre portáteis'],
    minStartYear: 1989,
  },
  {
    id: 'single-franchise',
    label: 'Franquia Singular',
    kicker: 'UMA SÉRIE. UMA VIDA.',
    difficulty: 'ESPECIAL',
    description: 'Seu primeiro jogo funda a única franquia da carreira. Depois dela, nunca mais existe outra IP própria.',
    rules: ['Primeiro jogo fixa a franquia', 'Sequels, spin-offs e remakes liberados', 'Nunca troca de série'],
  },
  {
    id: 'acquired',
    label: 'Estúdio Adquirido',
    kicker: 'SOB NOVA DIREÇÃO',
    difficulty: 'DIFERENTE',
    description: 'Você já começa comprado por uma empresa, com uma equipe pequena, estrutura melhor e briefs corporativos chegando de tempos em tempos.',
    rules: ['Começa com 3 funcionários', 'Controladora desde o primeiro dia', 'Encomendas corporativas recorrentes'],
  },
  {
    id: 'solo',
    label: 'Autor Solo',
    kicker: 'NINGUÉM MAIS TOCA NO CÓDIGO',
    difficulty: 'DIFÍCIL',
    description: 'Sem contratar, sem comprar estúdios e sem dividir produção. Você recebe eficiência extra, mas toda a carreira depende de uma pessoa.',
    rules: ['Contratação bloqueada', 'Subsidiárias bloqueadas', 'Projetos mais baratos e rápidos'],
  },
  {
    id: 'eternal-indie',
    label: 'Indie Eterno',
    kicker: 'PEQUENO POR ESCOLHA',
    difficulty: 'ESPECIAL',
    description: 'O estúdio pode crescer em gente, mas nunca vira uma fábrica AAA. Sua identidade vive em projetos micro e pequenos.',
    rules: ['Escala máxima: Pequeno', 'Sem comprar estúdios', 'Produção mais barata e ágil'],
  },
  {
    id: 'arcade-empire',
    label: 'Império Arcade',
    kicker: 'A FICHA NUNCA ACABA',
    difficulty: 'ESPECIAL',
    description: 'Só existe fliperama. Você atravessa o crash de 1982 e tenta manter o arcade relevante quando o resto da indústria muda de casa.',
    rules: ['Somente Fliperama / Arcade', 'Disponível desde 1980', 'Sobreviva às viradas do mercado'],
  },
  {
    id: 'anthology',
    label: 'Antologia',
    kicker: 'NUNCA A MESMA COISA DUAS VEZES',
    difficulty: 'ESPECIAL',
    description: 'Toda estreia precisa ser uma IP original nova. Nada de sequência, remake, remaster, port ou coletânea.',
    rules: ['Apenas jogos originais', 'Uma franquia nova por projeto', 'Catálogo sempre recomeça do zero'],
  },
])
...
export function createCareerMode(modeId = 'traditional') {
  return {
    id: gameModeForId(modeId).id,
    lockedFranchiseId: null,
    lockedFranchiseName: null,
...
export function modeAllowsProjectType(state, projectTypeId) {
  ...
  if (modeId === 'single-franchise') {
    if (!state.careerMode?.lockedFranchiseId) return projectTypeId === 'original'
    return projectTypeId !== 'original'
  }
  return true
}
...
export function modeProjectPayloadValid(state, payload) {
  ...
  const modeId = modeIdForState(state)
  if (modeId === 'anthology') {
    if (payload.franchiseId || (payload.sourceGameIds?.length ?? 0)) return false
  }

  if (modeId === 'single-franchise') {
    const locked = state.careerMode?.lockedFranchiseId
    if (!locked) {
      if ((payload.projectType ?? 'original') !== 'original' || payload.franchiseId || (payload.sourceGameIds?.length ?? 0)) return false
    } else {
      if (payload.franchiseId && payload.franchiseId !== locked) return false
      const sources = (payload.sourceGameIds ?? []).map(id => state.games.find(game => game.id === id)).filter(Boolean)
      if (sources.some(game => game.franchiseId !== locked)) return false
      if (['sequel', 'spinoff'].includes(payload.projectType) && payload.franchiseId !== locked) return false
    }
  }
  return true
}
...
function starterEmployee(state, index, random) {
  ...
  return {
    ...
    roleId: role.id,
    personalityId: randomChoice(PERSONALITIES, random).id,
    skill,
    potential: Math.min(94, skill + randomInt(8, 20, random)),
    salary: Math.round(role.salary * Math.max(.72, era.costMultiplier * .78) / 50) * 50,
    morale: randomInt(68, 84, random),
    energy: 100,
    loyalty: randomInt(68, 88, random),
    months: 0,
    projects: 0,
    awards: 0,
...
export function applyGameModeStart(state, options = {}, random = Math.random) {
  ...
  if (mode.id === 'acquired') {
    ...
    state.corporate.ownership = {
      ...
      acquiredYear: state.date.year,
      price: 0,
      founderPayout: 0,
      autonomy: state.studio.autonomy,
      retainedBrand: true,
      earnout: 0,
      careerStart: true,
    }
    ...
    if (relationship) {
      relationship.trust = 66
      relationship.status = 'parceiro preferencial'
    }
    state.history[0] = {
      ...state.history[0],
      title: `${company.name} já está na placa`,
      body: `O estúdio abre com três funcionários, uma controladora e ${state.studio.autonomy}% de autonomia. Os briefs vão chegar; o resto da carreira ainda é seu.`,
      highlight: true,
    }
  }

  if (mode.id === 'solo') {
    state.player.money = 18000
    state.history[0] = { ...state.history[0], title: 'Uma mesa. Uma pessoa.', body: 'Sem RH, sem aquisições e sem equipe paralela. Tudo que sair daqui passa pelas suas mãos.', highlight: true }
  }

  if (mode.id === 'eternal-indie') {
    state.player.money = 18000
    state.history[0] = { ...state.history[0], title: 'Pequeno por escolha', body: 'O estúdio pode contratar e crescer, mas a produção fica presa ao tamanho em que ideias estranhas ainda cabem numa sala.', highlight: true }
  }
