...
export function StatStrip() {
  ...
  return (
    <section className="stat-strip" aria-label="Estado da carreira">
      ...
      <PulseStat label="CAIXA" value={player.money} format={formatMoney} danger={player.money < 0} priority="core" />
      <PulseStat label="ENERGIA" value={player.energy} suffix="%" danger={player.energy < 20} priority="core" />
      <PulseStat label="ESTRESSE" value={player.stress} suffix="%" danger={player.stress > 70} inverse priority="core" />
...
function PulseStat({ label, value, format = Math.round, suffix = '', danger, inverse = false, priority }) {
